...
of author affiliations. The description should be understandable by non-experts, at the
level of a department colloquium. The description should be meaningful to an
...
new intelligence analyst who is not expert, at the 
level of a colloqium. Highlight typical applications. Highlight any dual use technologies that may be of concern to the United States
Government.
...
# 'image_url': 'https://upload.wikimedia.org/wikipedia/commons/d/d9/Langley_hypersonic_wind_tunnels.jpg', 'image_thumbnail_url': 'https://upload.wikimedia.org/wikipedia/commons/thumb/d/d9/Langley_hypersonic_wind_tunnels.jpg/100px-Langley_hypersonic_wind_tunnels.jpg'

# we should defnitely put in the iamages! we have the urls. ok. 

st.title("Example: Author specific")
#st.markdown("""
#* Hypersonic speed (speed that is highly supersonic), 高超音速, Гиперзвуковая скорость
#* Hypersonic flight (flight below 90 kilometres at speeds above Mach 5), 高超音速飛行試驗, Гиперзвуковой летательный аппарат
#* Hypersonic flow (speed that is highly supersonic), 高超音速, Гиперзвуковая скорость
...
def load_centroids_asat():
  ...
  #  df = pd.read_json(df.to_json())
    dg = pd.read_pickle('washburnecentroids2d.pkl.gz')
    #return dg
    return dg[dg.cluster != -1]

@st.cache_data()
def load_dftriple_asat():
    dg = pd.read_pickle('washburnedftriple2d.pkl.gz')
    return dg

@st.cache_data()
def load_dfinfo_asat():
    dg = pd.read_pickle('washburnedfinfo2d.pkl.gz')
    #return dg
    return dg[dg['cluster'] != -1]
